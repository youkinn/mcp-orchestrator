...
export const SUPPORT_CHECK_SYSTEM_PROMPT = [
  ...
  "系统给你四样内容：",
  "1) 用户问题；",
  "2) 生成轮注入的原文片段（带 [片段N] / ⟨Qn⟩ 标记，与生成轮所见完全一致）；",
  "3) 模型答案（含 [Qn] / [片段N] 引用指针）；",
  "4) 引用指针清单。",
  "判定对象是「答案对用户问题的回答」：答案须正面回答该问题，且其引用片段在语义上支撑该回答，才算 supported。",
  "逐条判定每条指针指向的片段是否在语义上支撑答案的结论与关键断言：",
  "- supported：答案正面回答了用户问题，且片段内容支撑该断言（允许中阿数字等价、别名称谓等语义等价形式，不要求字面重合）；",
  "- unsupported：断言了片段中没有的内容（含凭先验 / 史实补全），或片段与结论无关；答非所问（答案内容与问题核心无关 / 未触及问题，如问结局却答过程）同样判 unsupported；",
  "- uncertain：无法确定。",
  "overall 判定答案整体是否回答了用户问题且被注入片段集合支撑：答案正面回答且有支撑引用且核心断言成立 → supported；",
  "答非所问，或核心断言无任何片段支撑 → unsupported；无法确定 → uncertain。",
