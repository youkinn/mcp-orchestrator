...
// 风云三国知识问答：本地题库查询工具（走 agent localTools，不经 MCP）
...
async function main() {
  ...
  // 各场景独立 Agent：general 无工具；weather 连 MCP 工具；sango 知识问答走本地题库工具
  ...
  const sangoKnowledgeAgent = new Agent(transport, llmConfig, {
    ...
    localTools: {
      sango_query: async (args: Record<string, unknown>) => {
        ...
        const hit = sangoService.search(text);
        return {
          content: [
            {
              type: 'text',
              text: hit
                ? `题干：${hit.question.question}\n答案：${hit.answer}`
                : 'hit=false',
            },
          ],
        };
