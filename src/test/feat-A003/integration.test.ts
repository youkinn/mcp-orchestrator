...
import { SangoService, SANGO_NO_SESSION_PROMPT } from '../../sango.js';
...
/**
 * feat-A003 端到端集成用例：真实 server.ts + 真实 Agent + 真实 SangoService，
 * 只把 MCP transport 与 LLM 换成替身（零网络、不起真实 MCP Server）。
 * 与同目录 server.test.ts（StubAgent，只验 HTTP 契约）、agent-routing.test.ts
 * （直调 Agent，不起服务）互补：这里验「HTTP 请求 → 统一 Agent → 工具」
 * 整条链路上的验收标准 1-10。
 */
...
/** 与 index.ts 的 SANGO_QUERY_TOOL 同文（本地工具，不经 MCP） */
...
const MCP_TOOLS: MCPToolDefinition[] = [FORECAST_TOOL, ALERTS_TOOL];
const UNIFIED_TOOLS: MCPToolDefinition[] = [...MCP_TOOLS, SANGO_QUERY_TOOL];
const UNIFIED_TOOL_NAMES = UNIFIED_TOOLS.map((tool) => tool.name);
...
/** MCP 替身：只记录调用，不发真实 stdio 连接；可注入 callTool 异常 */
class MockTransport extends MCPTransport {
  ...
  override async callTool(
    ...
    return { content: [{ type: 'text', text: '晴，5℃' }] };
  }
}
...
class FakeModel {
  ...
  respond = async (
    ...
    this.calls.push({
      ...
      // 快照：Agent 会往同一个数组继续 push 工具结果，直接引用会读到后续轮次内容
      messages: [...messages],
    });
...
/** 按 index.ts 的装配方式起真实服务：单个统一 Agent + 本地题库工具 + createServer */
async function startApp(
  t: TestContext,
  options: HarnessOptions = {}
): Promise<Harness> {
  const dir = mkdtempSync(join(tmpdir(), 'a003-integration-'));
  const questionFile = join(dir, 'questions.json');
  writeFileSync(
    questionFile,
    JSON.stringify(options.questions ?? QUESTIONS),
    'utf8'
  );
  ...
  const sango = new SangoService({ questionFile });
  ...
  const agent = new Agent(transport, LLM_CONFIG, {
    ...
    tools: UNIFIED_TOOLS,
    localTools: {
      // 与 index.ts 的 sango_query handler 同形：本地召回，不经 MCP
      sango_query: async (args: Record<string, unknown>) => {
        if (options.localToolError) {
          throw options.localToolError;
        }
        const query = typeof args.text === 'string' ? args.text : '';
        const hits = sango.candidates(query);
        return {
          content: [
            {
              type: 'text',
  ...
  const app = createServer(agent, sango, { port: 0, allowedOrigin: '*' });
  ...
  return { baseUrl, transport, model, sango };
}
...
test('1/6 美国天气语义：只传 message，真实 Agent 调 get-forecast，200 信封回传播报文本', async (t) => {
  const answer = '纽约今天基本适合通勤，5℃有小雨。出门必备：手机、乘车码、证件、钥匙、雨伞';
...
test('1 多轮 tool-use：get-forecast 后 get-alerts 再出文本，调用顺序与参数完整透传', async (t) => {
  const answer = '纽约今天建议调整时间，5℃有大风预警。出门必备：手机、乘车码、证件、钥匙';
  const { baseUrl, transport, model } = await startApp(t, {
    script: [
      toolUse('get-forecast', { latitude: 40.71, longitude: -74.01 }),
...
test('2/6 题库内三国题（问法不同、含义相同）：走本地 sango_query，答案取题库原文，不经 MCP', async (t) => {
  const { baseUrl, transport, model } = await startApp(t, {
    script: [toolUse('sango_query', { text: '夏侯的字是什么' }), text('元让')],
  });
  ...
  assert.deepEqual(transport.callToolCalls, []);
  // 本地工具结果按「题干 → 答案」回填给模型，模型才有机会输出题库原文
  assert.ok(
    JSON.stringify(lastMessage(model.calls[1])).includes(
      '夏侯惇的字是什么？ → 元让'
    )
  );
});

test('3/6 题库未收录：sango_query 无候选，回答固定话术且不用题库外知识', async (t) => {
  const fixed = '题库未收录该题，请换个问法';
  const { baseUrl, transport, model } = await startApp(t, {
    questions: [],
    script: [
      toolUse('sango_query', { text: '司马懿的字是什么' }),
      text(fixed),
    ],
  });
...
test('5/6 闲聊：不调用任何工具，自由作答，不套天气与题库模板', async (t) => {
...
test('6 默认路径：天气、题库、兜底三种语义共用同一份统一提示词与同一份可见工具', async (t) => {
  const { baseUrl, model } = await startApp(t, {
    script: [
      ...
      toolUse('sango_query', { text: '夏侯惇的字是什么？' }),
      text('元让'),
      text('你好，我在。'),
    ],
  });
...
test('8 GET /api/tools：真实 Agent 上报 MCP 工具与本地 sango_query，不再回源 transport.listTools', async (t) => {
  ...
  assert.deepEqual(res.body.data.tools[2], SANGO_QUERY_TOOL);
  // 上报能力与模型可见能力同源：注入 tools 后不打 MCP
  assert.equal(transport.listToolsCount, 0);
});

test('9 POST /api/sango/random：出题、判对、判错、查答案、无会话，全程不调 Agent / LLM / MCP', async (t) => {
  const { baseUrl, transport, model, sango } = await startApp(t);
  ...
  const current = sango.getCurrentQuestion(sessionId);
  assert.ok(current);
...
test('503：MCP 工具失败经真实 Agent 包装为 ToolExecutionError，判工具服务暂不可用（不再由 scenario 反推）', async (t) => {
...
test('500：本地题库工具失败不被包装，判处理请求失败（503 只属于 MCP 链路）', async (t) => {
  const { baseUrl } = await startApp(t, {
    script: [toolUse('sango_query', { text: '夏侯惇的字是什么？' })],
    localToolError: new Error('question bank broken'),
  });
