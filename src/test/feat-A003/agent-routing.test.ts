...
/** A002 题库规则块原文：统一提示词必须逐字沿用，只改工具名与「什么时候适用」的措辞 */
const FENGYUNSANGUO_RULES = [
...
test("②③ 提示词·题库域：A002 五条规则原文照搬，限定风云三国招募武将问答题，未收录走固定话术", () => {
  for (const rule of FENGYUNSANGUO_RULES) {
...
test("提示词·优先级次序：先判断是否命中专用能力，命中则调工具，否则按域兜底", () => {
  ...
  const fengyunsanguoStep = UNIFIED_SYSTEM_PROMPT.indexOf("1. 意图是否命中风云三国");
  ...
  assert.ok(fengyunsanguoStep > order, "题库判断应在次序小节内");
  assert.ok(weatherStep > fengyunsanguoStep, "天气判断应排在题库之后");
...
test("提示词·孤儿常量清理：agent.ts 不再保留 A002 旧提示词（sango.ts 已删除）", () => {
...
test("模型可见工具 = options.tools（MCP 天气 + fengyunsanguo_query），不走 transport.listTools", async () => {
  ...
  assert.deepEqual(
    ...
    ["get-forecast", "get-alerts", "fengyunsanguo_query"]
  );
  assert.equal(transport.listToolsCount, 0, "注入 tools 后不应再查 transport");
});

test("⑧ listTools()：注入 tools 时返回模型可见列表（含 fengyunsanguo_query），与 processQuery 同源", async () => {
  const transport = new MockTransport([FORECAST_TOOL]);
  const seenTools: MCPToolDefinition[][] = [];
  const modelCaller = async (
    _messages: any[],
  ...
  assert.ok(
    reported.some((tool) => tool.name === "fengyunsanguo_query"),
    "/api/tools 上报必须含题库工具 fengyunsanguo_query"
  );
...
test("500 落点：localTools（fengyunsanguo_query）抛错原样上抛，不包装成 ToolExecutionError", async () => {
  const transport = new MockTransport([FENGYUNSANGUO_QUERY_TOOL]);
  const original = new Error("question bank broken");
  const modelCaller = async (): Promise<ModelResponse> =>
    toolUseResponse("fengyunsanguo_query", { text: "夏侯惇的字是什么？" });
...
test("② 模型要调 fengyunsanguo_query：走 localTools 而非 transport，text 传用户原始问题", async () => {
  const transport = new MockTransport(UNIFIED_TOOLS);
